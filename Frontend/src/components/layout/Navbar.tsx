...
export default function Navbar({ isExpanded, onToggle }: NavbarProps) {
    ...
    >
      <div className="flex h-full flex-col px-3 py-5">
        ...
        <div className="flex flex-col">
          <NavItem to="/lives" icon={<LuMonitor className="h-5 w-5" />} label="전체 방송" isExpanded={isExpanded} />
          <NavItem
            to="/category"
          ...
          />
          <NavItem to="/following" icon={<LuHeart className="h-5 w-5" />} label="팔로잉" isExpanded={isExpanded} />
        ...
        {user ? (
          ...
          <NavItem to="/login" icon={<LuLogIn className="h-5 w-5" />} label="로그인" isExpanded={isExpanded} />
        )}
      </div>
    </nav>
  );
