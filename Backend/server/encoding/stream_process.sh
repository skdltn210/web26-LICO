...
ffmpeg -rw_timeout 10000000 -i "rtmp://192.168.1.6:1935/$APP_NAME/$STREAM_KEY" -y\
    -map 0:v -map 0:a -c:a aac -b:a 128k -c:v libx264 -b:v:2 2800k -s:v:2 1280x720 -preset veryfast -g 60 -tune zerolatency -profile:v baseline \
    -map 0:v -map 0:a -c:a aac -b:a 128k -c:v libx264 -b:v:1 1200k -s:v:1 854x480 -preset veryfast -g 60 -tune zerolatency -profile:v baseline \
    -map 0:v -map 0:a -c:a aac -b:a 128k -c:v libx264 -b:v:0 600k -s:v:0 640x360 -preset veryfast -g 60 -tune zerolatency -profile:v baseline \
    -keyint_min 60  -hls_time 1 -hls_segment_type fmp4 -hls_flags delete_segments+independent_segments+append_list -hls_list_size 3 -hls_delete_threshold 5 \
